...
export const KNOBS: KnobDef[] = [
  { key: 'kLa_CO2', label: 'CO₂ stripping (kLa_CO2)', min: 0.04, max: 0.30, step: 0.005, group: 'co2',
    fmt: (v) => v.toFixed(3), hint: 'Higher = more CO₂ removed by sparging/agitation',
    info: 'Volumetric CO₂ mass-transfer coefficient. Low values let dissolved pCO₂ accumulate, acidifying the Golgi. This is the lumped term a CFD surrogate would replace with a scale-dependent field.' },
  { key: 'pH_set', label: 'pH setpoint', min: 6.8, max: 7.4, step: 0.01, group: 'co2',
    fmt: (v) => v.toFixed(2), info: 'Culture pH setpoint. Sets the baseline from which pCO₂ excursions perturb Golgi pH.' },
  { key: 'mu_max', label: 'Max growth rate μmax', min: 0.018, max: 0.038, step: 0.001, group: 'co2',
    unit: 'h⁻¹', fmt: (v) => v.toFixed(3), info: 'Cell-line/medium growth capacity. Faster growth shortens Golgi residence → less mature glycans.' },

  { key: 'Fglc', label: 'Glucose feed', min: 0.0, max: 0.6, step: 0.01, group: 'feed',
    fmt: (v) => v.toFixed(2), info: 'Bolus/continuous glucose feed rate. Sustains nucleotide-sugar precursor pools.' },
  { key: 'Fgln', label: 'Glutamine feed', min: 0.0, max: 0.12, step: 0.005, group: 'feed',
    fmt: (v) => v.toFixed(3), info: 'Glutamine feed. N source; drives ammonia and part of the UDP-GlcNAc pool.' },
  { key: 'Fgal', label: 'Galactose feed', min: 0.0, max: 0.25, step: 0.01, group: 'feed',
    fmt: (v) => v.toFixed(2), hint: 'Classic galactosylation booster',
    info: 'Galactose supplementation raises the UDP-Gal pool → higher galactosylation (with uridine + Mn²⁺ in practice).' },
  { key: 'asn_level', label: 'Asparagine availability', min: 0.1, max: 1.0, step: 0.05, group: 'feed',
    fmt: pct, hint: 'Depletion → high-mannose rises',
    info: 'Asparagine (often the primary N source in modern CHO media) feeds the hexosamine pathway → UDP-GlcNAc. Depletion late in culture is a documented driver of rising high-mannose.' },
  ...
  { key: 'Tset', label: 'Temperature', min: 30, max: 37.5, step: 0.5, group: 'env', unit: '°C',
    ...
    info: 'Culture temperature. A 37→33 °C downshift slows growth, lengthens effective Golgi residence, and raises galactosylation/sialylation (heat-transport / temperature-shift strategy).' },
  { key: 'DO', label: 'Dissolved O₂', min: 5, max: 100, step: 5, group: 'env', unit: '%',
    fmt: (v) => v.toFixed(0), hint: 'Low DO reduces sialylation',
    info: 'Dissolved oxygen (% air saturation). Low DO shifts metabolism and reduces oxidative precursor supply (CMP-sialic acid, UDP-Gal), lowering sialylation.' },

  { key: 'Mn', label: 'Mn²⁺ level', min: 0.0, max: 3.0, step: 0.1, group: 'gene',
    fmt: x, hint: 'Divalent-metal cofactor for GalT/SiaT',
    info: 'Manganese is the dominant divalent-metal cofactor for β-1,4-galactosyltransferase and others. Primary galactosylation-tuning lever; Mg²⁺/Zn²⁺/Co²⁺ contribute secondarily (trace-metal drift is a real lot-to-lot CMC issue).' },
  { key: 'MGAT', label: 'MGAT expression', min: 0.2, max: 3.0, step: 0.1, group: 'gene',
    fmt: x, info: 'GlcNAc-transferase (GnTI/II) — gates entry into the complex pathway; low MGAT raises high-mannose.' },
  { key: 'B4GALT', label: 'B4GALT1 expression', min: 0.2, max: 4.0, step: 0.1, group: 'gene',
    fmt: x, info: 'β-1,4-galactosyltransferase. Overexpression raises galactosylation (→ CDC potency).' },
  { key: 'FUT8', label: 'FUT8 expression', min: 0.0, max: 2.0, step: 0.05, group: 'gene',
    fmt: x, hint: 'Knockdown → afucosylation → ADCC',
    info: 'α-1,6-fucosyltransferase (core fucose). Knockout/knockdown is the classic route to afucosylated, high-ADCC antibodies.' },
  { key: 'ST6GAL', label: 'ST6GAL1 expression', min: 0.2, max: 5.0, step: 0.1, group: 'gene',
    fmt: x, info: 'α-2,6-sialyltransferase. Overexpression raises sialylation (→ serum half-life, anti-inflammatory).' },
];
