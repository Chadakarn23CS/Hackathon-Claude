...
export const KNOBS: KnobDef[] = [
  ...
  { key: 'Fglc', label: 'Glucose feed', min: 0.0, max: 0.6, step: 0.01, group: 'feed',
    unit: 'norm.', fmt: (v) => v.toFixed(2), hint: 'Normalized feed rate (baseline 0.20)',
    info: 'Glucose feed rate in normalized model units — the engine runs on normalized nucleotide-sugar pools, so this is a dimensionless rate relative to baseline (0.20), not an absolute g/L. Sustains precursor pools.' },
  { key: 'Fgln', label: 'Glutamine feed', min: 0.0, max: 0.12, step: 0.005, group: 'feed',
    unit: 'norm.', fmt: (v) => v.toFixed(3), hint: 'Normalized feed rate (baseline 0.035)',
    info: 'Glutamine feed rate in normalized model units (dimensionless, relative to baseline 0.035 — the model has no absolute concentration scale). N source; drives ammonia and part of the UDP-GlcNAc pool.' },
  { key: 'Fgal', label: 'Galactose feed', min: 0.0, max: 0.25, step: 0.01, group: 'feed',
    unit: 'norm.', fmt: (v) => v.toFixed(2), hint: 'Normalized rate — galactosylation booster',
    info: 'Galactose supplementation rate in normalized model units (dimensionless, baseline 0; no absolute concentration scale). Raises the UDP-Gal pool → higher galactosylation (with uridine + Mn²⁺ in practice).' },
