...
1. **Ammonia → Golgi pH → galactosylation coupling.** `golgi_pH()` currently responds to CO₂ only. Vil-II models ammonia raising Golgi pH (Henderson–Hasselbalch, Eq. 28) and Vil-I shows high ammonia → nongalactosylated forms. The model already tracks ammonia in the bioreactor; wiring it into Golgi pH would close a documented mechanism. **(Recommended.)**
